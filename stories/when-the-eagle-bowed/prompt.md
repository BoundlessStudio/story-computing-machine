# Prompt

## Prompt

> [WP] The last Eagle Queen was executed a century ago, yet when a wild eagle lands before an orphaned huntress and bows, ancient armor begins clawing its way out of the mountain—and every king in the valley orders her killed before she remembers who she was.

> update the story "When the Eagle Bowed" from giving up the crown and disappearing to defeating the enemies with the help of the armor and taking the kingdom completing the Eagle Queen’s ancient revenge but unleassing some even more terifing; see the image for the reference fo bookend story with the Eagle full power on display.

> keep in mind i going to another story to end this one based on the attach image so we need to keep it open ended; lets that endbookend close it.

> the greater terror most be her Eagle shadow from the armor.

> "this story will end as it begins acting beyond her control" that is good as we use the other story to control it and find love. and rebuild the kingdom...

## Constraints

- Target length: 2,500–4,000 words
- POV, tense, and distance: derive from what the story should reveal, distort,
  conceal, or make intimate unless specified
- Tone and audience: broadly accessible unless specified
- Required: a complete story that resolves its central promise
- Craft profile: prospective-2026-08-08
- Avoid: none specified

## Rewrite request

> I want you to review and rewrite all dialog in no locked stories should based on the new improvements to the writing pipeline.

## Rewrite reference images

- None supplied for this rewrite.

## Rewrite constraints

- Cover: AUTO
- Craft profile: prospective-2026-08-21
- Authority: the rewrite request controls where it conflicts with the original prompt; all unaffected original requirements remain binding.

## Reference images

- codex-clipboard-ac97df8e-840f-4019-a228-ac069b37d0b0.png — supplied with the conquest-and-revenge request.
- codex-clipboard-1258888e-1797-40e3-974b-41a9af579585.png — resupplied with the open-ending clarification.
- Role: visual direction for the later bookend's full-power Eagle. This story completes Yavren's conquest and ancient revenge, then releases the Eagle shadow from her armor as the unresolved greater terror. The later bookend owns its full-power finale, the struggle to control it, finding love, and rebuilding the kingdom.
- Audience and intensity: broadly accessible dark fantasy; non-graphic violence and supernatural horror.
