...
and aliases to the stories or legacy sources that use them so new work can avoid
accidental reuse.
...
- Legacy, abandoned, and non-canon names remain searchable reservations unless
  a row explicitly marks them released. Listing a name here never promotes it.
...
2026-07-23. The registry includes one-off and referential names, cover
identities, titles used as names, named animals, and named person-like
entities. It does not invent placeholders for characters the source leaves
unnamed.
...
- `Lena` — Lena Mercer (`S03`) and Lena Venn (`S01-O`).
- `Mara` — Mara from Hinge (`S08`) and Mara Osset (`S07`).
- `Nisha` — external legacy `S02` and `S04`.
- `Pell` — Orrin Pell (`S01-O`) and Pell (`S07`).
- `Voss` — M. Voss (`S03`) and Rhea Voss (`S01-O`); the existing ruling says
  they are not connected.

These entries are warnings for future adaptation, not permission to repeat the
names again.
