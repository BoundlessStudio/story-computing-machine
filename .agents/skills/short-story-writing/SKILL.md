...
Under `prospective-2026-08-23`, also use the Story section's `Dialogue promise`,
`Dialogic medium`, and `Dialogue engine`, plus the Voice capsule's sixth field:

- `Dialogue promise` defines the relationship, tonal, and communication
  experience this prompt owes the reader; it is not a line-count target.
- `Dialogic medium` deliberately selects speech, writing, internal voices,
  signs, interfaces, gesture, silence, failed contact, or a story-specific mix.
- `Dialogue engine` states why these people must communicate and how that
  communication produces movement rather than merely reporting it.
- `Relationship movement` states what each major participant wants from
  another, cannot comfortably request, and what changes between them.

For 08-23, rhetorical ownership includes humor, evasion, affection, misreading,
social authority, and willingness to listen as well as reasoning and precision.
Do not let professional competence become the only source of distinction. When
the prompt and people support it, leave room for play, error, pettiness,
embarrassment, distraction, poor explanation, and social unevenness.
...
Treat non-spoken contact as dialogue when it carries relational action. For
each major exchange, ask what changes in knowledge, leverage, trust,
commitment, risk, relationship, or action. More short exchanges do not repair a
scene whose participants only transfer operational information.
...
6. **Competence test:** If everyone becomes an articulate professional solving
   a material problem, restore the character-specific limits, play, mistakes,
   status, distraction, or bad explanation that the prompt actually supports.
7. **Ending-gesture test:** If the final contact becomes practical instructions,
   resumed maintenance, inventory, breakfast, or a return to work, keep it only
   when that gesture completes this story's promise rather than avoiding the
