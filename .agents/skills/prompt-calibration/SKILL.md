...
1. Read `prompt-scout/data/latest-ranked.json`, `prompt-scout/taste-profile.md`,
   `prompt-scout/data/preferences.json`, recent feedback, and
   `prompt-scout/data/calibration-asked.jsonl` when present.
...
4. Write `prompt-scout/data/pending-calibration.json`:
...
5. Run `scripts/record-prompt-calibration-set.ps1`. It validates the selection,
   appends the asked ledger, and creates `prompt-scout/calibration/latest.md`.
...
1. Resolve the user's complete ordering against the archived calibration set
   ...
   newer set.
2. Run `scripts/record-prompt-calibration-response.ps1 -CalibrationId <id>
   -Order ...`.
...
4. Review the new ordering with earlier explicit feedback. Update
   `prompt-scout/taste-profile.md` only with evidence-backed patterns, including
   support count, contradictions, and confidence.
